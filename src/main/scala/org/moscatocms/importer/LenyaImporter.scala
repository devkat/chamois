...
import org.moscatocms.web.HtmlLinkRewriter
...
object LenyaImporter extends FileImporter {
  ...
  def getDocs(area:File)(xmlNode:scala.xml.Node): Set[Doc] = {
    val docs = (xmlNode \ "@uuid") match {
      ...
      case n => {
        println("Found doc " + (xmlNode \ "@id").text)
  ...
  def importVersion(doc:Resource, docFolder:File, lang:String, fileName:String)(implicit doc2uuid:Map[Doc, UUID]) = {
    ...
    def newVersion(mt:MediaType, content:Array[Byte]) {
      doc.setMediaType(mt)
      println("    Importing version (%s)".format(fileName))
      doc.newVersion(content)
    }
    
    MediaType.parse(mediaType) match {
      case Some(MediaType("application", "xhtml+xml")) => newVersion(MediaType("text", "html"),
        (<html lang={lang}>
          <head>
      ...
      //case Some(mType) if doc.currentVersionId => newVersion(mType, read(contentFile))
      case _ => {}
    }
